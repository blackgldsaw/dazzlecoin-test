...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Dazzlecoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"dazzlecointools.com", "dzc.su"},
    {"5.101.61.170", "192.168.10.185"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Dazzlecoin is probably already running."), addrBind.ToString().c_str());
        else
