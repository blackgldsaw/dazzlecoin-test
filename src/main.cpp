...
#if defined(NDEBUG)
# error "Dazzlecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x9427d17f93ff18415354b5feced328548c35ffae0c2602b694babb5036bbc4ca");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Dazzlecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Dazzlecoin Signed Message:\n";
...
{
    // Dazzlecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Dazzlecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Dazzlecoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Dazzlecoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Dazzlecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Dazzlecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9427d17f93ff18415354b5feced328548c35ffae0c2602b694babb5036bbc4ca");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Lifestealer is the best hero of dota 2";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("ab01d84df853e8584dd808c2925f01cebeb788e99632f2e78c4514bf1b37a6aa") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1498133396;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1680656;
        ...
        assert(block.hashMerkleRoot == uint256("0x6accfb8a1ef8eca95b83a54bbf60da019e01bad99aadadd77f560c33898ca073"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Dazzlecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DazzlecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DazzlecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DazzlecoinMiner(CWallet *pwallet)
{
    printf("DazzlecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("dazzlecoin-miner");
    ...
    try { loop {
        ...
        printf("Running DazzlecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DazzlecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DazzlecoinMiner, pwallet));
}
